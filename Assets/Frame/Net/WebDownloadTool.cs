...
namespace Farme.Net
{
    ...
    public class WebDownloadTool 
    {
        ...
        /// <param name="donwloadProgressCallback">下载进度回调</param>
        public static void DownloadFile(string url, UnityAction<byte[]> resultCallback, UnityAction<float> donwloadProgressCallback = null)
        {
            ShareMono.GetSingleton().StartCoroutine(IEDownloadFile(url, resultCallback, donwloadProgressCallback));
        }
        private static IEnumerator IEDownloadFile(string url,UnityAction<byte[]> resultCallback,UnityAction<float> donwloadProgressCallback = null)
        {
            ...
            while (!uwrao.isDone)
            {
                ...
                donwloadProgressCallback?.Invoke(uwrao.progress);
                yield return uwrao.progress;
            }
            donwloadProgressCallback?.Invoke(1);
            resultCallback?.Invoke(www.downloadHandler.data);        
        }
        #endregion
        ...
        /// <param name="url">地址</param>
        /// <param name="resultCallback">结果回调</param>
        public static void WebDownloadText(string url, UnityAction<string> resultCallback,UnityAction<float> donwloadProgressCallback=null)
        {
            ShareMono.GetSingleton().StartCoroutine(IEWebDownloadText(url, resultCallback, donwloadProgressCallback));
        }
        private static IEnumerator IEWebDownloadText(string url, UnityAction<string> resultCallback, UnityAction<float> donwloadProgressCallback=null)
        {
            ...
            {
                donwloadProgressCallback?.Invoke(uwrao.progress);
                yield return uwrao.progress;//等待异步请求完成
            }
            donwloadProgressCallback?.Invoke(1);
